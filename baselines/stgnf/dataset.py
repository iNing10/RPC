...
from common.data import (
    BasePoseDataset,
    is_shanghaitech_hr_skip,
    is_ubnormal_family,
    list_pose_json_names,
    make_data_loader,
...
class PoseSegDataset(BasePoseDataset):
    """
    ...
    def __init__(self, path_to_json_dir, path_to_vid_dir=None, normalize_pose_segs=True, return_indices=False,
                 ...
                 **dataset_args):
        super().__init__(dataset_args.get("dataset", "ShanghaiTech"))
...
def get_dataset_and_loader(args, trans_list, only_test=False):
    ...
    if args.vid_res is not None:
        dataset_args['vid_res'] = args.vid_res
    dataset, loader = dict(), dict()
    splits = ['train', 'test'] if not only_test else ['test']
    for split in splits:
        evaluate = split == 'test'
        split_seed = args.seed if not evaluate else args.seed + 1
...
def shanghaitech_hr_skip(shanghaitech_hr, scene_id, clip_id):
    ...
    if is_shanghaitech_hr_skip(f"{scene_id}_{clip_id}"):
        return True
    return False
...
def gen_dataset(person_json_root, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True,
    ...
    data_dir = dataset_args.get("data_dir", "data/")
    json_list = list_pose_json_names(
        person_json_root,
        dataset=dataset,
        data_root=data_dir,
        split=split_name_from_path(person_json_root),
        specific_clip=num_clips,
    )
