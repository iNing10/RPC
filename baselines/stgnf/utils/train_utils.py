...
def init_model_params(args, dataset):
    use_xy_only = not args.model_confidence
    return {
        'pose_shape': dataset["test"][0][0][:2].shape if use_xy_only else dataset["test"][0][0].shape,
        ...
        'use_ms_temporal': args.use_ms_temporal,
        'ms_dilations': parse_ms_dilations(args.ms_dilations),
        'device': args.device,
    }


def parse_ms_dilations(value):
    if isinstance(value, (list, tuple)):
        dilations = tuple(int(item) for item in value)
    else:
        dilations = tuple(int(item.strip()) for item in str(value).split(",") if item.strip())
    if not dilations:
        raise ValueError("ms_dilations must contain at least one integer")
    if any(dilation <= 0 for dilation in dilations):
        raise ValueError("ms_dilations must contain positive integers only")
    return dilations
