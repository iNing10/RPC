...
from utils.hr_ubnormal_protocol import (
    apply_hr_ubnormal_eval_mask,
    dataset_root_name as canonical_dataset_root_name,
    filter_hr_ubnormal_files,
    is_hr_ubnormal,
    is_ubnormal_family,
...
def _resolve_dataset_root(args, dataset_name):
    root_name = {
        'ShanghaiTech-HR': 'ShanghaiTech',
    }.get(dataset_name, canonical_dataset_root_name(dataset_name))
    dataset_root = os.path.join(args.data_dir, root_name)
    return dataset_root


def apply_dataset_eval_masks(gt_arr, scores_arr, clip_names, args=None):
    return gt_arr, scores_arr


def score_dataset(score, metadata, args=None):
    gt_arr, scores_arr, clip_names = get_dataset_scores(score, metadata, args=args)
    scores_arr = smooth_scores(scores_arr)
    gt_arr, scores_arr = apply_dataset_eval_masks(gt_arr, scores_arr, clip_names, args=args)
    if args is not None and getattr(args, "dump_scores_npz", False):
        out_dir = args.dump_scores_dir
    elif args is not None and getattr(args, "exp_dir", None):
        out_dir = os.path.join(args.exp_dir, "results")
    else:
        out_dir = "."
    ...
    print(len(scores_arr), "clips scored.")
    print(len(gt_arr), "frames scored.")
    gt_np = np.concatenate(gt_arr)
    scores_np = np.concatenate(scores_arr)
    print(len(gt_np), "frames scored.")
...
def get_dataset_scores(scores, metadata, args=None):
    ...
    if is_ubnormal_family(args.dataset):
        dataset_root = _resolve_dataset_root(args, 'UBnormal')
        ...
        clip_list = filter_hr_ubnormal_files(
            args.dataset, args.data_dir, split_name_from_path(pose_segs_root), clip_list
        )
        per_frame_scores_root = os.path.join(dataset_root, 'gt')
    else:
        dataset_root = _resolve_dataset_root(args, args.dataset)
        per_frame_scores_root = os.path.join(dataset_root, 'gt', 'test_frame_mask')
    ...
    for clip in tqdm(clip_list):
        ...
        if clip_score is not None:
            ...
            dataset_clip_names.append(clip)
    ...
    return dataset_gt_arr, dataset_scores_arr, dataset_clip_names
...
def get_clip_score(scores, clip, metadata_np, metadata, per_frame_scores_root, args):
    ...
    if not is_ubnormal_family(args.dataset):
    ...
    if is_hr_ubnormal(args.dataset):
        split = split_name_from_path(args.pose_path["test"])
        clip_gt, clip_score = apply_hr_ubnormal_eval_mask(args.data_dir, split, clip, clip_gt, clip_score)
        if len(clip_gt) == 0:
            return None, None
