...
def get_stgcn(in_channels, hidden_channels, out_channels,
              ...
              use_ms_temporal=False, ms_dilations=(1, 2, 3)):
    ...
    if hidden_channels == 0:
        block = nn.ModuleList((
            st_gcn(
                ...
                1,
                residual=(not first),
                use_ms_temporal=use_ms_temporal,
...
class FlowStep(nn.Module):
    def __init__(
        ...
        if A is None:
            g = Graph(strategy=strategy, max_hop=max_hops)  # 使用默认的openpose布局
...
class STG_NF(nn.Module):
    ...
    def prior(self, data, label=None):
        ...
        mean, logs = split_feature(h, "split")
        return mean, logs
    ...
    def forward(
        ...
        if reverse:
            ...
            return self.normal_flow(x, label, return_latent=return_latent)

    def normal_flow(self, x, label, return_latent=False):
        b, c, t, v = x.shape
        z, logdet = self.flow(x, reverse=False)
        
        # Compute standard Gaussian prior
        mean, logs = self.prior(x, label=label)
        log_pz = gaussian_likelihood(mean, logs, z)
        
        # Combine logdet and prior for complete objective
